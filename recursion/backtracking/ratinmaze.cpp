...
void printAllPath(int maze[][4], int row, int col, int srcx, int srcy, string &output, vector<vector<bool>> &visited) {
	...
	int newx = srcx-1;
	int newy = srcy;
	if(isSafe(newx,newy,maze,row,col,visited)) {//mark visited
		visited[newx][newy] = true;//call recursion
		output.push_back('U');
		printAllPath(maze, row, col, newx, newy, output , visited );//backtracking
		output.pop_back();
		visited[newx][newy] = false;
	}
...
int main() {
//scr is the starting position of rat
	int maze[4][4] = {
	{1,0,0,0},
	{1,1,0,0},
	{1,1,1,0},
